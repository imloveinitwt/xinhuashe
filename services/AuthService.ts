...
export const AuthService = {
  ...
  login: async (email: string, role?: UserRole): Promise<User> => {
    ...
    // 简单模拟：实际后端会校验密码 hash
    // 这里我们允许通过邮箱匹配，或者如果是演示账号直接通过
    let user = users.find((u: any) => u.email === email || u.name === email);

    // 如果没找到，但在演示模式下，我们会自动创建一个临时的基于角色的用户
    if (!user && role) {
      user = AuthService.createMockUser(role, email.split('@')[0]);
    }
   ...
   * 注册逻辑
   */
  register: async (username: string, email: string, role: UserRole = 'creator'): Promise<User> => {
    await delay(800);
