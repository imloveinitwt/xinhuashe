                         ...
                         <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
                             <div className="flex justify-between items-center mb-6">
                                <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                                   <BookOpen className="w-5 h-5 text-indigo-600" /> 成功案例
                                </h3>
                                <button 
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {MOCK_PROJECT_CASES.slice(0, 2).map(caseItem => (
                                   <div key={caseItem.id} className="border border-slate-100 rounded-xl overflow-hidden hover:shadow-md transition-shadow group cursor-pointer" onClick={() => setActiveTab('cases')}>
                                      {/* Cover */}
                                      <div className="h-48 relative overflow-hidden">
                                         <img src={caseItem.coverImage} alt={caseItem.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                         ...
                                         <div className="absolute bottom-3 left-3 text-white">
                                            ...
                                            <h4 className="font-bold text-lg leading-tight line-clamp-1">{caseItem.title}</h4>
                                         </div>
                                      </div>
                                      
                                      {/* Body */}
                                      <div className="p-5">
                                         <p className="text-slate-600 text-sm line-clamp-2 mb-4 h-10">{caseItem.description}</p>
                                         
                                         {/* Results */}
                                         <div className="grid grid-cols-2 gap-2 mb-4 bg-slate-50 p-3 rounded-lg">
                                            {caseItem.results.map((res, i) => (
                                               <div key={i}>
                                                  <div className="text-indigo-600 font-bold text-lg">{res.value}</div>
                                                  <div className="text-slate-400 text-[10px] uppercase font-bold">{res.label}</div>
                                               </div>
                                            ))}
                                         </div>
                                         ...
                                         {caseItem.clientTestimonial && (
                                            <div className="flex gap-2 items-start text-xs text-slate-500 italic">
                                               <Quote className="w-3 h-3 text-indigo-300 flex-shrink-0" />
                                               <span className="line-clamp-2">"{caseItem.clientTestimonial.text}"</span>
                                            </div>
                                         )}
                                      </div>
                                   </div>
